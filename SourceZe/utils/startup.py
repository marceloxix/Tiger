    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await mody.tgbot.send_file(
                ...
                caption="**‏۞︙ بــوت تايجر  يـعـمـل بـنـجـاح ✓ \n۞︙ أرسل `.الاوامر`لرؤية اوامر السورس \n  ۞︙ لأستعمال بوت الأختراق عبر كود التيرمكس أرسل`.هاك`**",
                buttons=[(Button.url("سورس تايجر ", "https://t.me/PP_KU"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def add_bot_to_logger_group(chat_id):
    ...
    try:
        ...
        try:
            ...
            LOGS.error(str(e))
#by @PP_KU بس اشوفك خامطه للكود اهينك وافضحك
SourceZe = {"@PP_KU", "@PP_UF"}
async def saves():
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        descript = "- عزيزي المستخدم هذه هي مجموعه الاشعارات يرجى عدم حذفها  - @PP_KU"
        photobt = await mody.upload_file(file="mody/razan/resources/start/Ze.JPEG")
        _, groupid = await create_supergroup(
            "مجموعة السجل تايجر  ", mody, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            " مجموعة التخزين تايجر ", mody, Config.TG_BOT_USERNAME, descript, photobt
        )

...
def main_menu():
    text = f"**- [سـورس تايجر ](https://t.me/PP_KU)\
        \n المسـاعد\
        \n\
        \n◽  تايجر  لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("mody"):
            buttons = [
                (
                    ...
                    Button.url(" SourceTige UsᴇʀBoᴛ", "https://t.me/PP_KU"),
                )
            ]
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس  تايجر  🧸♥",
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس  تايجر  🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس  تايجر  🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/PP_KU"),
                Button.url(
                    "كـروب  المـساعدة 📬",
                    "https://t.me/PP_UF",
                ),
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت  تايجر  🧸♥](https://t.me/PP_KU)**\
            \n\
            \n❤  تايجر  هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [جروب المسـاعدة](https://t.me/PP_UF)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),

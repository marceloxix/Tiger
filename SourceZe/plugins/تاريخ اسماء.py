# Copyright (C) 2023 SourceTiger TEAM
# FILES WRITTEN BY  @XIX_A
import asyncio
...
async def _(event):  #   : @XIX_A
    "To get name/username history."
    input_str = "".join(event.text.split(maxsplit=1)[1:])
    reply_message = await event.get_reply_message()
    if not input_str and not reply_message:
        await edit_delete( #ترجمه فريق تايجر على التيلكرام
            event,
    ...
    for i in check:
        ...
        else:#ترجمه فريق تايجر على التيلكرام
            jasem = True

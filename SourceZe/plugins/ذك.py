...
async def mody_gpt(event):
    ...
    if not zilzal and event.reply_to_msg_id and zzz.text: #code by t.me/XIX_A
        mody = zzz.text
    if not event.reply_to_msg_id: #code by t.me/XIX_A
    ...
    async with borg.conversation(chat) as conv: #code by t.me/XIX_A
        try:
            ...
            ahmed = sourceze.text
            if "another 8 seconds" in sourceze.text: #code by t.me/XIX_A
            ...
            malath = ze.text
            if "understanding" in ze.text: #code by t.me/XIX_A
                ...
                return await borg.send_message(event.chat_id, aa)
            await zed.delete()
            await borg.send_message(event.chat_id, f"**السؤال : {mody}\n\n{malath}**\n\n───────────────────\n𝐓𝐈𝐆𝐄𝐑 𝐔𝐒𝐄𝐑𝐁𝐎𝐓**** 𝗧**ᴏᴏʟꜱ**\n\t\t\t\t\t\t\t\t@PP_KU • ᴼᵖᵉⁿᴬᴵ")
        except YouBlockedUserError: #code by t.me/XIX_A
            ...
            if "understanding" in ze.text: #code by t.me/XIX_A
                ...
                await event.delete()
                return await borg.send_message(event.chat_id, aa)
            if "Please wait a moment" in ze.text: #code by t.me/XIX_A
                await asyncio.sleep(5)
                ze = await conv.get_response()
                malath = ze.text
            await zed.delete()

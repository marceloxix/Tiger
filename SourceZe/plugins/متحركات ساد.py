...
#جميع الحقوق محفوظة لسـورس تايجر  تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    XIX_A = await reply_id(event)
    if sad:
        mody = f"**˛ 𝙏𝙄𝙂𝙀𝙍 ، ٰ𝖦𝗂𝖿 𝖲ِ𝖺ٰ𝖣 . .**\n"
        mody += f"✛━━━━━━━━━━━━━✛\n"
        mody += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=mody, reply_to=XIX_A)

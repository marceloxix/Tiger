    ...
    async with bot.conversation("@SpamBot") as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=7131944336)
            )
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="۞︙ BY : @PP_KU 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@mody.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="۞︙ THEME BY : @PP_KU 🎊",parse_mode="html")
  await theme.delete()
